...
from .forms import *
from a_posts.models import LikedPost
...
def profile_view(request,username=None):
    ...
    if request.htmx:
        if 'top-posts' in request.GET:
            ...
            # Get LikedPost objects for this user, then extract the posts
            liked_posts_objects = LikedPost.objects.filter(user=profile.user).select_related('post').order_by('-created')
            posts = [liked_post.post for liked_post in liked_posts_objects]
